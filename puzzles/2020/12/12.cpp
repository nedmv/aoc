// Puzzle is available at https://adventofcode.com/2020/day/12
#include "../2020.hpp"
using namespace std;
...
std::string y2020::solve_12a(const std::vector<std::string> &input) {
  Ship ship = Ship();
  return to_string(ship.navigate(const_cast<std::vector<std::string>&>(input)));
}

std::string y2020::solve_12b(const std::vector<std::string> &input) {
  WShip ship = WShip();
  return to_string(ship.navigate(const_cast<std::vector<std::string>&>(input)));
}